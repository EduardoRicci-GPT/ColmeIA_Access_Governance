...
| 15 | ~~Camada de IA sobre o assurance~~ — **feito**, ADR-0027 | feito pelo avesso: agrupar, priorizar e atribuir causa já eram determinísticos, então sobrou a FORMA, e só para dois dos quatro públicos. Cascata de faculdades com o núcleo determinístico no degrau ZERO, e ancoragem que descarta — não corrige — prosa que inventa identificador ou número |
| 15b | Primeira faculdade de linguagem real | `FaculdadeDeBancada` é bancada. A primeira faculdade de verdade, local ou remota, é integração; a porta já está no lugar e uma instalação sem nenhuma funciona inteira |
| 15c | Taxa de fabricação medida por faculdade | o campo existe e a ancoragem produz o dado; nada acumula a série. Sem ela, escolher entre duas faculdades é preferência, não medição — o que a doutrina da origem recusa |
| 15e | Conferir o porte contra a fonte primária | `Ricci-I-Next/MPEH_Sinergentia-` esteve fora do alcance da sessão que escreveu o ADR-0027: a doutrina foi lida no espelho da Aletheia. Três pontos podem mudar — as três negativas da identidade, a taxonomia de tarefas e as classes de âncora. Exige sessão iniciada com aquele repositório como fonte |
| 15d | Calibrar orçamento do ciclo e circuito | são palpites declarados e ainda não passaram pelo registro de calibragem, ao contrário dos pesos do Health Score |
