...
`npm run verificar` — **670 verificações, todas passando** neste repositório.

São 672 no checkout da Aletheia, e a diferença não é defeito: duas verificações
...
| Cenários hospitalares, exceções, competência, explicação e emergência | 213 |
