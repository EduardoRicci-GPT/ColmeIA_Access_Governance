...
`npm run verificar` — **638 verificações, todas passando** neste repositório.

São 640 no checkout da Aletheia, e a diferença não é defeito: duas verificações
...
| Cenários hospitalares, exceções, competência, explicação e emergência | 181 |
