...
`npm run verificar` — **574 verificações, todas passando** neste repositório.

São 576 no checkout da Aletheia, e a diferença não é defeito: duas verificações
...
| Cenários hospitalares, exceções operacionais e competência | 120 |
